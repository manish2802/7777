...
import com.absa.model.FileInfo;

@Controller
public class DownloadFileStoreController {
	...
	public String getListFiles(Model model) {
		List<FileInfo> fileInfos = fileStorage.loadFiles().map(path -> {
			...
			String url = MvcUriComponentsBuilder
					.fromMethodName(DownloadFileStoreController.class, "downloadFile", path.getFileName().toString()).build()
					.toString();
